...
parfor i = 1:Runs
    ...
    while T <= tMax
        ...
        aProtein = xCurrentmRNA(:,1) .* kP .* xCurrentLocalPools;
        aProteinDecay = xCurrentProtein .* gammap;
        
        %local pool propensities
        for j = 1:NumGenes
            aLocalPools(j) = Ribosomes.*kIN.*mRNATotals(j);
            if mRNATotals(j) == 0
                aLocalPools(j+NumGenes) = xCurrentLocalPools(j).*kOUT;
            else
                aLocalPools(j+NumGenes) = xCurrentLocalPools(j).*kOUT./mRNATotals(j);
            end
        end
