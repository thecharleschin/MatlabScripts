...
%ParamArray = [.05,.1,1,5,10]; %alpha Value
%ParamArray = [.0005,.001,.01,.1,.5]; %kB Value
%ParamArray = [0,.1,1,10,100]; %kRb Value
%ParamArray = [0,.1,1,10,100]; %kRR Value
%ParamArray = [.1,.5,1,5,10]; %kOFF Value
%ParamArray = [.1,.5,1,5,10]; %halflife Value
...
for n = 1:length(ParamArray)
    ...
    kON = ParamArray(n);%gene burst on rate
...
for i = 1:length(ParamArray)
    name = sprintf('%s %g',testParam,ParamArray(i));
    linestore(i) = plot(binInterval,AllbinRalt(:,i),'marker','o','markersize',8,'markerfacecolor'...
        ,c(i,:),'markeredgecolor','k','displayname',name,'color',c(i,:));
end

set(gca,'YScale','log');
legend(linestore,'location','southeast')
...
name = sprintf('AltBSBSParam%s.jpg',testParam);
saveas(gcf,name)
